...
3. (On Player Level)
Sometimes a single player could make huge impact on basketball. We choose three most popular players, Stephen Curry, Lebron James and Giannis Antetokounmpo to investigate their shooting choices and look at how other variables could influence their shooting decision such as in differnet periods of game.

## League Shooting Pattern Analysis

### Distribution of Distance of Shots
...
### Shot Distance of Missed Shots and Made Shots

```{r}
boxplot(shot_distance~event_type,data=shot_data,horizontal=TRUE,main="Shot Distance for shots missed and made",col="lightblue",border="black",las=2,ylab='',cex.axis=0.5)
```
...
```{r}
shot_data %>%
  ggplot(aes(x= shot_zone_area, y = minutes_remaining)) + 
  geom_boxplot()+
  facet_wrap(~event_type)+
  coord_flip()+
  labs(title = "Boxplots of Shot Zones agains Minutes Remaining", x = "Shot Zone", y = "Minutes Remaining")
  
```
In order to be more specific, we dig into different zones based on minutes remaining in the period. From the boxplot above, surprisingly, except Back Court shots (which sometimes only happen when there is no time remaining), the distribution of missed shots are almost the same between different zones. However, the distribution of shots made in different zones varies and it is more likely that players made shots in left side center when time remaining is sufficient.

### Shot Attempted in Different Periods

Every NBA game has four regular periods to decide which team is winner (there would over time periods if score is tied), we want to see that how well players shoot in different periods.
...
mosaic(event_type~shot_type + period, direction = c("v","v","h"), shot_mos,
       highlighting_fill = c("cornflowerblue","grey80"),
       labeling = labeling_border(rot_labels = c(0,0,0)),
       main = "Mosaic Plot")
```

As shown in mosaic plot above, for the  2-point field goals, players perform very similar in different periods. For the 3-point field goals, shots made verall declined significantly from the 2-pointers and the field goal percentage varies slightly from period to period. We could see that in first period the percentage of shots made is highest and last one has lowest. This is a normal phenomenon to us because players would be too exhausted to shoot well near the end of the game. However, one interesting thing is that players seem to perform better in third period than second period. Perhaps it is because players have longer break for halftime so that they could recover and shoot better in third quarter.
...
period_shots%>%
  ...
  ggtitle("Alluvial Diagram of Percentage of 3-points Change")+
  theme_void()+
